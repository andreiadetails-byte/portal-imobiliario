...
export let lastTranslateError = null;

export async function translateText(text, targetLangCode) {
  if (!text || !text.trim()) return '';
  if (!process.env.DEEPL_API_KEY) {
    lastTranslateError = 'DEEPL_API_KEY não está definida no servidor.';
    console.error(lastTranslateError);
    return text;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      lastTranslateError = `Erro do DeepL (${res.status}): ${errText}`;
    ...
    lastTranslateError = `Erro de rede ao contactar o DeepL: ${err.message}`;
