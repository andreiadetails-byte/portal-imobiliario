...
// Impede que texto escrito por utilizadores (nome, mensagem, etc.) seja
// interpretado como código HTML dentro do email — sem isto, alguém podia
// escrever algo tipo "<script>" no nome ou na mensagem de contacto, e esse
// código corria no leitor de email de quem o recebesse. Usa sempre esta
// função em qualquer texto que venha de um formulário preenchido por uma
// pessoa, antes de o colocar dentro do HTML do email.
...
export function propertyCardHtml({ typology, address, price, businessType, photoUrl, propertyId }) {
  const priceLabel = price
    ...
    : '';
  const safeTypology = escapeHtml(typology);
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F1E8D6; border-radius:8px; overflow:hidden; margin: 14px 0;">
      <tr>
        ...
        <td style="width:120px; vertical-align:top;">
          <img src="${escapeHtml(photoUrl)}" alt="${safeTypology}" width="120" height="90" style="display:block; width:120px; height:90px; object-fit:cover;">
        </td>` : ''}
        <td style="padding: 12px 14px; vertical-align:middle;">
          <div style="font-size:13.5px; font-weight:700; color:#332E22;">${safeTypology} · ${safeAddress}</div>
          ${priceLabel ? `<div style="font-size:13px; color:#5A6B49; font-weight:600; margin-top:4px;">${priceLabel}</div>` : ''}
          ${propertyId ? `<div style="margin-top:6px;"><a href="${SITE_URL}/property/${encodeURIComponent(propertyId)}" style="font-size:12px; color:#5A6B49;">Ver anúncio completo →</a></div>` : ''}
        </td>
      </tr>
    </table>
  `;
