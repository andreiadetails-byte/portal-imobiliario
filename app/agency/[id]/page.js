  ...
  return (
    <div>
      ...
      <div className="wrap" style={{ paddingTop: 0 }}>
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-end', marginTop: -40, paddingBottom: 24, borderBottom: '1px solid var(--line)', flexWrap: 'wrap' }}>
          <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="meta">
              {profile.account_type === 'agencia' ? 'Agência imobiliária' : 'Particular'}
              {profile.agency_license && ` · Licença ${profile.agency_license}`}
            </div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <b className="display" style={{ fontSize: 20 }}>{properties.length}</b>
            <div className="meta">Imóveis ativos</div>
          </div>
        </div>

        <h2 className="display" style={{ fontSize: 20, margin: '28px 0 16px' }}>Imóveis</h2>

        {properties.length === 0 && <p className="empty-state">Ainda não tem imóveis ativos.</p>}
