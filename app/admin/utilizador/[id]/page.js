          ...
          {properties.map((p) => {
            ...
            return (
              <div key={p.id} className="card" style={{ padding: 16, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                {firstPhoto ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={firstPhoto} alt="" style={{ width: 72, height: 56, objectFit: 'cover', borderRadius: 5, flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 72, height: 56, borderRadius: 5, background: 'linear-gradient(135deg, var(--azulejo), #4A5A3C)', flexShrink: 0 }} />
                )}
