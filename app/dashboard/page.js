  ...
  return (
    <div className="wrap" style={{ padding: '40px 32px 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <h1 className="display" style={{ fontSize: 28 }}>Olá, {profile?.full_name || 'utilizador'}</h1>
        <div>
          <Link href="/publish" className="btn btn-primary" style={{ marginRight: 12 }}>+ Novo anúncio</Link>
          <button onClick={handleLogout} className="btn">Sair</button>
        </div>
      </div>

      <h2 className="display" style={{ fontSize: 18, marginBottom: 14 }}>Os meus anúncios</h2>
      {properties.length === 0 ? (
        <p className="empty-state">Ainda não publicaste nenhum imóvel.</p>
      ) : (
        <div className="card" style={{ marginBottom: 40 }}>
          {properties.map((p) => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', borderBottom: '1px solid var(--line)' }}>
              ...
              <Link href={`/property/${p.id}`} style={{ fontSize: 13, color: 'var(--telha)' }}>Ver anúncio</Link>
            </div>
          ))}
        </div>
      )}

      <h2 className="display" style={{ fontSize: 18, marginBottom: 14 }}>Leads recentes</h2>
      {leads.length === 0 ? (
        <p className="empty-state">Ainda não recebeste nenhuma lead.</p>
      ) : (
        <div className="card">
          {leads.map((l) => (
            <div key={l.id} style={{ padding: '14px 18px', borderBottom: '1px solid var(--line)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 12,
                  ...
                  {l.status === 'novo' ? 'Por responder' : 'Respondido'}
                </span>
              </div>
