...
function ChatInner() {
  ...
  return (
    <div className="wrap" style={{ padding: '40px 32px 80px' }}>
      <h1 className="display" style={{ fontSize: 26, marginBottom: 20 }}>Mensagens</h1>

      {conversations.length === 0 ? (
        <div className="empty-state">Ainda não tens conversas. Envia uma mensagem a partir da página de um imóvel.</div>
      ) : (
            ...
            {conversations.map((c) => (
              ...
              >
                ...
                <div style={{ fontSize: 11.5, color: 'var(--text-soft)', marginTop: 2 }}>
                  {c.buyer_id === user.id ? 'Conversa com o anunciante' : 'Conversa com interessado'}
                </div>
              </div>
            ))}
          </div>
