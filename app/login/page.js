...
export default function LoginPage() {
  ...
  async function handleSignup(e) {
    ...
    // Atualiza o tipo de conta (particular/agência) no perfil criado automaticamente pelo trigger
    if (data.user) {
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '60px 24px' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: 20 }}>
        <Link href="/" style={{ fontSize: 13, color: 'var(--text-soft)' }}>&larr; Voltar à página inicial</Link>
      </p>
    </div>
  );
}
